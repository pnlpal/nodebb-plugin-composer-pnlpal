...
define("composer/drafts", ["api", "alerts"], function (api, alerts) {
  ...
  drafts.init = function (postContainer, postData) {
    ...
    postContainer.on(
      "keyup",
      "textarea, input.handle, input.title, input.external-link",
  ...
  function saveDraft(postContainer, draftIconEl, postData) {
    if (
      ...
      const externalLinkEl = postContainer.find("input.external-link");
      const externalLink =
        externalLinkEl && externalLinkEl.length && externalLinkEl.val();
      ...
      if (
        ...
        if (postData.action === "topics.post") {
          ...
          draftData.externalLink = externalLink;
          ...
          draftData.externalLink = externalLink || postData.externalLink;
        }
        if (!app.user.uid) {
    ...
    require(["composer"], function (composer) {
      if (draft.action === "topics.post") {
        composer.newTopic({
          ...
          externalLink: utils.escapeHTML(draft.externalLink),
          body: draft.text,
        ...
        composer.editPost({
          ...
          externalLink: draft.externalLink
            ? utils.escapeHTML(draft.externalLink)
            : undefined,
          body: draft.text,
        });
      }
