...
plugin.init = async function (data) {
  ...
  routeHelpers.setupAdminPageRoute(
    router,
    "/admin/plugins/composer-pnlpal",
    controllers.renderAdminPage
  );
};

plugin.appendConfig = async function (config) {
  config["composer-pnlpal"] = await meta.settings.get("composer-pnlpal");
  return config;
};

plugin.addAdminNavigation = async function (header) {
  header.plugins.push({
    route: "/plugins/composer-pnlpal",
    icon: "fa-edit",
    name: "Composer (pnlpal)",
  });
  return header;
};
...
plugin.addPrefetchTags = async function (hookData) {
  ...
  hookData.links = hookData.links.concat(
    prefetch.map((path) => ({
      rel: "prefetch",
      href: `${nconf.get("relative_path") + path}?${
...
function generateDiscardRoute(req, topicData) {
  if (req.query.cid) {
    return `${nconf.get("relative_path")}/category/${validator.escape(
